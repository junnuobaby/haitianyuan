                                ...
                                <div class="panel-body">
                                    <div class="col-md-5">
                                        <form class="form-horizontal" onkeydown="if(event.keyCode==13)return false;">
                                            ...
                                            <a class="btn btn-danger theme-bg-color buy_stock_btn" id="buy">买入
                                            </a>
                                        </form>
                                    </div>
        ...
        function get_hint_list(event) {
            ...
            if ((event.which >= 48 && event.which <= 57) || event.which == 8 || event.which == 46
                ...
                if (code_input_val.length >= 1) {
                    xhr = $.ajax({
                        url: '<?php echo base_url("index.php/stocks/hint_list"); ?>' + '/' + code_input_val,
                        method: 'get',
                        cache: false,
                        dataType: 'json',
            ...
            //输入的是非数字
                ...
                if (code_input_val.length >= 1) {
                    xhr = $.ajax({
                        url: '<?php echo base_url("index.php/stocks/hint_chinese"); ?>' + '/' + encodeURIComponent(code_input_val),
                        method: 'get',
                        cache: false,
                        dataType: 'json',
                        success: show_hint_list
...
//                }, 1000); //每隔1s自动请求一次
        ...
        function selected_code_info(code) {
            $.ajax({
                url: '<?php echo base_url("index.php/stocks/code_info"); ?>' + '/' + code,
                method: 'get',
                cache: false,
                dataType: 'json',
        ...
        //显示所选证券的实时数据信息
        function code_info_display(response) {
            var bond_name = response.name; //ajax获取证券名称
            var bond_cur_price = response.new; //获取最新价
            var bond_lastday_price = response.last; //获取昨日收盘价
            var bond__highest = response.up; //涨停
            var bond_lowest = response.down; //跌停
            var sell_1ist = response.sell; //卖五
            var buy_1ist = response.buy; //买五
        ...
        $('#buy').click(function () {
            ...
            if (confirm(info_str)) {
                $.ajax({
                    url: '<?php echo base_url("index.php/stocks/buy_bond"); ?>',
                    method: 'post',
